...
TOKEN_DATA = {
    'access_token': 'ya29.a0AfB_byAgOb8NzJZdyIXW8Av9wJpOTv7TGshQyOgP4gEEQt_-PWTaLIG2ymPu7e17HUF32yG32cYs_-DFUMIS78GObaQbqU69hSgQpWvZYJ7J8ogSPCrEuCFFQEI6KRyG6w60p5pmcVNXzakRy665HtuYh82m0teJsyDLaCgYKAdQSARASFQHGX2Mit_rsCojKaTt-8RsiPViMMg0171',
    'expires_in': 3599,
...
def notify_user(email, contract_number):
    ...
    # Create the email message
    msg = MIMEMultipart()
    msg.attach(MIMEText(f'Новое сообщение в контракте {contract_number}'))
    msg['to'] = email
    msg['subject'] = 'TenderLine: Новое сообщение'

    # Send the email
    raw_message = {'raw': base64.urlsafe_b64encode(msg.as_string().encode('utf-8')).decode('utf-8')}
    try:
        message = service.users().messages().send(userId='me', body=raw_message).execute()
        print('Email sent successfully!')
    except HttpError as error:
