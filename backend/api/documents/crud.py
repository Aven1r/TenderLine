...
async def generate_pdf(document: schemas.Document):
    ...
    story.append(Paragraph(f"""1.1. Поставщик обязуется поставить, а Заказчик обязуется оплатить в обусловленный срок – Товар в соответствии с условиями договора и Спецификацией к нему (Приложение №2).""", style))
    ...
    story.append(Paragraph(f"2.1. Цена настоящего договора {document.contract_price} рублей, 00 коп. без налога (НДС)." , style))
